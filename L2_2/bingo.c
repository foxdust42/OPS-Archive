...
void child_work(int n, mqd_t pin, mqd_t pout)
{
    ...
    while (life--)
    {
        if (TEMP_FAILURE_RETRY(mq_receive(pout, (char*)&ni, 1, NULL)) < 0)
        {
            ERR("mq_recieve");
        }
        printf("[%d], Recieved %d\n", getpid(), ni);
        if (my_bingo == ni)
        {
            if (TEMP_FAILURE_RETRY(mq_send(pin, (const char *)&my_bingo, 1, 1)) < 0)
            {
                ERR("mq_send");
            }
            return;
     ...
     if (TEMP_FAILURE_RETRY(mq_send(pin, (const char *)&n, 1, 0)) < 0)
     {
        ERR("mq_send");
     }
}
...
void parent_work(mqd_t pout){
    ...
    while (children_left)
    {
        ...
        if (TEMP_FAILURE_RETRY(mq_send(pout, (const char *)&ni, 1, 0 ))<0)
        {
            ERR("mq_send");
        }
        sleep(1);
